...
import { buildCreditsReport } from './creditsReport'
...
import type { AnimationName, ApesJob, CharacterManifest, Direction, ExtractedPart, KitbashRecipe } from './types'
...
export async function renderRecipeFrameToDataUrl({
  ...
  await renderRecipeFrameToContext({
    context,
    recipe,
    characters,
    partLibrary,
...
export async function buildFullPackageManifest(
  ...
  return {
    ...
    credits_report: buildCreditsReport(character, recipe, partLibrary, lpcCatalog),
...
export function downloadCreditsReport(
  ...
  downloadBlob(
    ...
    new Blob([JSON.stringify(buildCreditsReport(character, recipe, partLibrary, lpcCatalog), null, 2)], { type: 'application/json' }),
  )
}
